...
async def main() -> None:
    ...
    # @dp.inline_query(query="https://www.youtube.com/*")
    # @dp.inline_query(query="https://youtu.be/*")
    @dp.inline_query(F.query.startswith("https://www.youtube.com/"))
    @dp.inline_query(F.query.startswith("https://youtu.be/"))
    async def video_sender(query):
        logging.info(f"InlineQuery: {query.query}")
        print()
        # vid_info = yt.extract_info(query.text, download=False)
        ...
        #         file_dl = yt_dl.extract_info(query.text)
